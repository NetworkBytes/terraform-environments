...
module "Syslog" {
  ...
  # hostname eg dev2-syslog1-1
  names = "${var.environment}-syslog${var.site}-1"
  
  
  #IPs for instancees
  ip_addresses    = "${lookup("ip_addresses_syslog", "${var.environment}_site${var.site}")}"
  ip_gateways     = "${lookup("ip_gateways_syslog",  "${var.environment}_site${var.site}")}" 
